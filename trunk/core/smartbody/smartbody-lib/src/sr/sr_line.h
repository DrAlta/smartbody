...
class SrLine
 { public :
   ...
   public :
    ...
    SBAPI SrLine ( const SrPnt &v1, const SrPnt &v2 ) : p1(v1), p2(v2) {}

    /*! Set endpoints. */
    SBAPI void set ( const SrPnt &v1, const SrPnt &v2 ) { p1=v1; p2=v2; }

    /*! Same as copy operator. */
    SBAPI void set ( const SrLine &l ) { p1=l.p1; p2=l.p2; }

    /*! Copy operator from another SrLine. */
    SBAPI void operator = ( const SrLine &l ) { p1=l.p1; p2=l.p2; }
    ...
    SBAPI bool intersects_triangle ( const SrPnt &v0, const SrPnt &v1, const SrPnt &v2,
    ...
    SBAPI bool intersects_square ( const SrPnt& v1, const SrPnt& v2,
    ...
    SBAPI int intersects_box ( const SrBox& box, float& t1, float& t2, SrPnt* vp=0 ) const;
    ...
    /*! Returns 1 or 2 if the line intersects the sphere (center,radius), otherwise
        ...
        to SrLine::p1 */
    SBAPI int intersects_sphere ( const SrPnt& center, float radius, SrPnt* vp=0 ) const;

    /*! Returns the closest point in the line to p. Parameter k, if given,
        will be such that: closestpt == p1+k*(p2-p1) */
    SBAPI SrPnt closestpt ( SrPnt p, float* k=0 ) const;

    /*! Outputs in format: "p1 p2". */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, const SrLine& l );

    /*! Inputs from format: "p1 p2". */
    SBAPI friend SrInput& operator>> ( SrInput& in, SrLine& l );
