...
class SrTriangle
 { public :
   ...
   public :
    ...
    SBAPI void set ( const SrPnt& p1, const SrPnt& p2, const SrPnt& p3 ) { a=p1; b=p2; c=p3; }

    /*! Returns k, such that a*k.x + b*k.y + c*k.z == p. */
    SBAPI SrVec barycentric ( const SrPnt &p ) const;
    ...
    SBAPI void translate ( const SrVec &k, const SrVec& v );

    /*! Apply the translation v to each triangle vertex. */
    SBAPI void translate ( const SrVec& v );

    /*! Returns the normalized triangle normal. */
    SBAPI SrVec normal () const;

    /*! Outputs in format: "a,b,c". */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, const SrTriangle& t );

    /*! Inputs from format: "a,b,c". */
    SBAPI friend SrInput& operator>> ( SrInput& in, SrTriangle& t );
 };
