...
class SrPolygon : public SrArray<SrPnt2>
 { private :
   ...
   public :
    ...
    SBAPI void grow ( float radius, float maxangrad );

    /*! Get centroid of polygon */
    SBAPI SrPnt2 centroid () const;

    /*! Reverse the order of the elements */
    SBAPI void reverse ();

    /*! Adds dv to each vertex of the polygon */
    SBAPI void translate ( const SrVec2& dv );

    /*! Rotates the polygon around center about angle radians */
    SBAPI void rotate ( const SrPnt2& center, float radians );
    ...
    SBAPI SrPnt2 south_pole ( int* index=0 ) const;

    /*! Returns the convex hull in pol */
    SBAPI void convex_hull ( SrPolygon& pol ) const;
    ...
    SBAPI int pick_vertex ( const SrPnt2& p, float epsilon );
    ...
    SBAPI int pick_edge ( const SrPnt2& p, float epsilon, float& dist2 ) const;

    /*! Divides the polygon in triangles */
    SBAPI void ear_triangulation ( SrArray<SrPnt2>& tris ) const;
    ...
    SBAPI void get_configuration ( float& x, float& y, float& a ) const;
    ...
    SBAPI void set_configuration ( float x, float y, float a );

    /*! Tests if the polygon intersects with the given segment. */
    SBAPI bool intersects ( const SrPnt2& p1, const SrPnt2& p2 ) const;
    ...
    SBAPI friend int sr_compare ( const SrPolygon* p1, const SrPolygon* p2 );

    /*! Outputs the open flag and calls the SrArray::method */
    SBAPI friend SrOutput& operator<< ( SrOutput& out, const SrPolygon& p );

    /*! Check for the open flag and calls the SrArray::method */
    SBAPI friend SrInput& operator>> ( SrInput& inp, SrPolygon& p );
 };
