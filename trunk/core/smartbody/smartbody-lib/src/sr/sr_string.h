...
class SrString
 { private:
   ...
   public:
    ...
    SBAPI SrString ( char c, int len=1 );
    ...
    SBAPI SrString& set ( const char *st );
    ...
    SBAPI SrString& setf ( const char *fmt, ... );
    ...
    /*! Puts in xi and xf the indices of the first and last non-white 
        ...
        char or not. */
    SBAPI void bounds ( int &xi, int &xf ) const;
   ...
   /*! Put in s the substring defined by positions inf and sup.
    ...
    SBAPI void get_substring ( SrString& s, int inf, int sup ) const;
   ...
   /*! Put in s the next sequence of characters (a string) found in SrString.
    ...
    SBAPI int get_next_string ( SrString& s, int i ) const;
    ...
    SBAPI int search ( const char* st, bool ci=true ) const;
    ...
    SBAPI int get_file_name ( SrString& filename ) const;
    ...
    SBAPI int extract_file_name ( SrString& filename );
    ...
    SBAPI int remove_file_extension ();
    ...
    SBAPI int get_file_extension ( SrString& ext ) const;
    ...
    /*! Search for an extension at the end of the string, put it in ext, and
        ...
        changed. */
    SBAPI int extract_file_extension ( SrString& ext );
    ...
    SBAPI bool has_file_extension ( const char* ext ) const;
    ...
    SBAPI bool make_valid_path ( const char* s );
    ...
    SBAPI static bool has_absolute_path ( const char* s );

    /*! Member version of the has_absolute_path() static method */
    SBAPI bool has_absolute_path () const { return has_absolute_path(_data); }
    ...
    SBAPI operator const char* () const { return _data; }
    ...
    SBAPI char& operator[] ( int i ) { return _data[i]; }
    ...
    SBAPI const char& get ( int i ) const { return _data[i]; }
    ...
    SBAPI void append ( const char* st );
    ...
    SBAPI void insert ( int i, const char* st );
    ...
    SBAPI void remove ( int i, int dp );
    ...
    /*! Replaces the first occurence of oldst with newst,
        ...
        If ci is true (the default), the used compare function is case-insensitive. */
    SBAPI int replace ( const char* oldst, const char* newst, bool ci=true );

    /*! Calls the method replace until -1 is returned.
        Returns the number of replacements done. */
    SBAPI int replace_all ( const char* oldst, const char* newst, bool ci=true );
    ...
    /*! Makes SrString be exactly the same object as b, and then makes b an 
        ...
        up many algorithms. */
    SBAPI void take_data ( SrString &s );
    ...
    SBAPI char* leave_data ( char*& s );

    /*! Appends st to the SrString by calling append(). */
    SBAPI SrString& operator << ( const char* st );
    SBAPI SrString& operator << ( int i );
    SBAPI SrString& operator << ( float f );
    SBAPI SrString& operator << ( double d );
    SBAPI SrString& operator << ( char c );

    /*! Assignment operator by calling set(). */
    SBAPI SrString& operator = ( const SrString &s );
    ...
    /*! Compares strings using sr_compare(const char*,const char*), that does 
        ...
        and >0 if s1>s2. */
    SBAPI friend inline int sr_compare ( const SrString* s1, const SrString* s2 ) { return ::sr_compare(s1->_data,s2->_data); }
    ...
    SBAPI friend inline bool operator == ( const SrString& s1, const char* s2 ) { return sr_compare(s1._data,s2)==0? true:false; }
    SBAPI friend inline bool operator == ( const char* s1, const SrString& s2 ) { return sr_compare(s1,s2._data)==0? true:false; }
    SBAPI friend inline bool operator == ( const SrString& s1, const SrString& s2 ) { return sr_compare(s1._data,s2._data)==0? true:false; }
