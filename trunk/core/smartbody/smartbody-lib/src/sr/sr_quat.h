...
class SrQuat
 { public :
   ...
   public :
    ...
    SBAPI SrQuat () : w(1.0f), x(0), y(0), z(0) {}

    /*! Constructor from 4 floats (qw,qx,qy,qz). Implemented inline. */
    SBAPI SrQuat ( float qw, float qx, float qy, float qz ) : w(qw), x(qx), y(qy), z(qz) {}

    /*! Constructor from 4 floats (w,x,y,z) from a float array. Implemented inline. */
    SBAPI SrQuat ( const float* f ) : w(f[0]), x(f[1]), y(f[2]), z(f[3]) {}

    /*! Copy constructor. Implemented inline. */
    SBAPI SrQuat ( const SrQuat& q ) : w(q.w), x(q.x), y(q.y), z(q.z) {}
    ...
    SBAPI SrQuat ( const SrVec& axis, float radians ) { set(axis,radians); }
    ...
    SBAPI SrQuat ( const SrVec& axisangle ) { set(axisangle); }

    /*! Initializes SrQuat with the rotation from v1 to v2.
         The method set() is called inline in this constructor. */
    SBAPI SrQuat ( const SrVec& v1, const SrVec& v2 ) { set(v1,v2); }

    /*! Initializes SrQuat from a rotation matrix */
    SBAPI SrQuat ( const SrMat& m ) { set(m); }
    ...
    /*! Set the four values */
    SBAPI void set ( float qw, float qx, float qy, float qz ) { w=qw; x=qx; y=qy; z=qz; }

    /*! Set the four values from a float pointer */
    SBAPI void set ( const float* f ) { w=f[0]; x=f[1]; y=f[2]; z=f[3]; }

    /*! Defines SrQuat as the rotation from v1 to v2. */
    SBAPI void set ( const SrVec& v1, const SrVec& v2 );

    /*! Defines SrQuat as the rotation around axis of the given angle in radians.
        Axis is internaly normallized. */
    SBAPI void set ( const SrVec& axis, float radians );
    ...
    SBAPI void set ( const SrVec& axisangle );

    /*! Defines SrQuat by extracting the rotation from the given rotation matrix. */
    SBAPI void set ( const SrMat& m );

	SBAPI void set ( float ex, float ey, float ez);

    /*! Gets the current axis and angle of rotation (in radians) that SrQuat defines. */
    SBAPI void get ( SrVec& axis, float& radians ) const;

    /*! Returns the rotation axis. */
    SBAPI SrVec axis () const;

    /*! Returns the angle in radians. */
    SBAPI float angle () const;

	SBAPI SrVec axisAngle() const;

	SBAPI SrVec getEuler() const;

    /*! Normalizes the quaternion and ensures w>=0 */
    SBAPI void normalize ();

    /*! Returns the inverse quaternium. */
    SBAPI SrQuat inverse() const;
    ...
    SBAPI void get_rot_mat ( float& a, float& b, float& c,
    ...
    SBAPI friend bool operator == ( const SrQuat &q1, const SrQuat &q2 );

    /*! Comparison operator makes an exact comparison, of the quaternion components. */
    SBAPI friend bool operator != ( const SrQuat &q1, const SrQuat &q2 );

    /*! Swaps the contents of q1 with q2. */
    SBAPI friend void swap ( SrQuat &q1, SrQuat &q2 );

    /*! Returns the interpolation between q1 and q2 with parameter t.
        Parameter q1 is not const because it is normalized. */
    SBAPI friend SrQuat slerp ( SrQuat &q1, const SrQuat &q2, float t );

    /*! Outputs data in "axis x y z ang a" format. */
    SBAPI friend SrOutput& operator<< ( SrOutput& out, const SrQuat& q );

    /*! Input data from "axis x y z ang a" format. */
    SBAPI friend SrInput& operator>> ( SrInput& in, SrQuat& q );

	SBAPI void setData(int i, float val) { if (i == 0) w = val; else if (i == 1) x = val; else if (i == 2) y = val; else if (i == 3) z = val; }

	SBAPI float getData(int i) { if (i == 0) return w; else if (i == 1) return x; else if (i == 2) return y; else if (i == 3) return z; else return 0;}
 };
