...
class SrOutput
 { public :
   ...
   public : 
    ...
    SBAPI SrOutput ( void(*f)(const char*,void*), void* udata );
    ...
    SBAPI void init ( void(*f)(const char*,void*), void* udata );

    /*! Returns the type of the output. Implemented inline. */
    SBAPI Type type () const { return _type; }
    ...
    SBAPI int margin () const { return _margin; }

    /*! Changes the current margin being used. */
    SBAPI void margin ( int m ) { _margin=m; }

    /*! Set margin character, default is '\t' */
    SBAPI void margin_char ( char c ) { _margin_char=c; }

    /*! returns the current margin character. */
    SBAPI char margin_char () const { return _margin_char; }
    ...
    SBAPI void incmargin ( int inc ) { _margin+=inc; }
    ...
    SBAPI void fmt_int ( const char* s );
    SBAPI void fmt_float ( const char* s );
    SBAPI void fmt_double ( const char* s );

    SBAPI const char* fmt_int () { return _intfmt; }
    SBAPI const char* fmt_float () { return _floatfmt; }
    SBAPI const char* fmt_double () { return _doublefmt; }
    ...
    SBAPI void putf ( const char *fmt, ... );

    /*! Outputs a formatted error message and calls exit(1). */
    SBAPI void fatal_error ( const char *fmt, ... );

    /*! Outputs a formatted warning message, but does not call exit(). */
    SBAPI void warning ( const char *fmt, ... );

    /*! Outputs a string using the put() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, const char *st );

    /*! Outputs a SrString using the put() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, const SrString& st );

    /*! Outputs a character using the put() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, char c );

    /*! Outputs a srbyte using intfmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, srbyte c );

    /*! Outputs an int using intfmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, int i );

    /*! Outputs a bool as "true" or "false" with the put() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, bool b );

    /*! Outputs a short int using intfmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, short s );

    /*! Outputs a sruint using intfmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, sruint i );

    /*! Outputs a float using floatfmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, float f );

    /*! Outputs a double using doublefmt with the putf() method. */
    SBAPI friend SrOutput& operator<< ( SrOutput& o, double d );
 };
