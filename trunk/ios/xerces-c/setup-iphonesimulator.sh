...
# This script is used for compiling xerces-c-3.1.1 for iphone simulator, other version not tested
# Modify variables if needed

export SBROOT=""
export PREFIX="$SBROOT/ios/xerces-c/iphonesimulator"
...
if [ ! -d "$SBROOT" ]; then
    ...
    ./configure --host="i386-apple-darwin" --prefix=$PREFIX --disable-dependency-tracking
    make clean
    make
    make install
fi
