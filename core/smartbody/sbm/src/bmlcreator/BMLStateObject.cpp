...
BMLStateObject::BMLStateObject() : BMLObject()
{
	...
	modeAttr->setValidValues(modes);	createStringAttribute("x", "", "", "Basic", 30, false, false, false, "Parameter X, would be used in 1D|2D|3D space.");
	...
	SmartBody::StringAttribute* loopAttr = createStringAttribute("sbm:wrap-mode", "", "", "Basic", 20, false, false, false, "Wrap mode for current state. Loop or Once.Default is Loop");
	std::vector<std::string> loops;
	loops.push_back("Loop");
	loops.push_back("Queued");
	loopAttr->setValidValues(loops);
	SmartBody::StringAttribute* startNowAttr = createStringAttribute("sbm:schedule-mode", "", "", "Basic", 20, false, false, false, "Schedule mode for the state. Now or Queued.Default is Queued");
	std::vector<std::string> startNow;
	startNow.push_back("Now");
	startNow.push_back("Queued");
	startNowAttr->setValidValues(startNow);
	SmartBody::StringAttribute* additiveAttr = createStringAttribute("sbm:blend-mode", "", "", "Basic", 20, false, false, false, "Blend mode for current state. Overwrite or Additive.Default is Overwrite");
	...
	createStringAttribute("sbm:partial-joint", "", "", "Basic", 20, false, false, false, "Starting joint name inside skeleton hierarchy for additive blending.");
