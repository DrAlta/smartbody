...
:	is_valid( false ),
	...
	forward_velocity( 0 ),
//	radius( 0 ),
	bi_world_rot(-1), bi_loco_vel_x(-1), bi_loco_vel_y(-1), bi_loco_vel_z(-1), bi_loco_rot_global_y(-1), bi_loco_rot_local_y(-1), bi_id(-1) 
{}
...
SkChannelArray& MeCtNavigationCircle::controller_channels() {
	if( request_channels.size() == 0 ) {
		...
		request_channels.add( SkJointName( SbmCharacter::LOCOMOTION_VELOCITY ), SkChannel::XPos ); //  1
		request_channels.add( SkJointName( SbmCharacter::LOCOMOTION_VELOCITY ), SkChannel::YPos ); //  2
		request_channels.add( SkJointName( SbmCharacter::LOCOMOTION_VELOCITY ), SkChannel::ZPos ); //  3
		...
		request_channels.add( SkJointName( SbmCharacter::LOCOMOTION_ID ), SkChannel::YPos ); //  6
	}
