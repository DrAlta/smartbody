...
# This script is used for compiling apr-1.3.* for iphone simulator, apr-1.4.5 doesn't work
# Modify variables if needed

export SBROOT=""
export PREFIX="$SBROOT/ios/activemq/apr/iphonesimulator"
...
if [ ! -d "$SBROOT" ]; then
    ...
    ./configure --host="i386-apple-darwin" --prefix=$PREFIX --disable-shared
    make clean
    make
    make install
fi
